...
extern "C" __global__ void gpu_compute_harmonic_angle_forces_kernel(Scalar4* d_force,
    ...
    curand_init(timestep*N*6*(idx+1), 0, 0, &localState);
    //printf("\ntimestep = %i   idx = %i   state = %i\n", timestep,idx,&state);
		...
		for (int angle_idx = 0; angle_idx < n_angles; angle_idx++)
			...
			//printf("\ntimestep = %i  idx = %i  angle_idx = %i  R1 = %f  R6 = %f   pitch = %f\n",timestep,idx,angle_idx,R1,R6,pitch);
			...
			*/
			...
			else if (cur_angle_abc == 1 && timestep != 0)
			{
				...
				force_idx.w = sqrt(force_idx.x*force_idx.x+force_idx.y*force_idx.y+force_idx.z*force_idx.z);
				//if (idx==337)
				//printf("\ntimestep = %i  idx = %i  angle_idx = %i  R1 = %f  R6 = %f   cur_angle_type = %i   cur_angle_abc = %i   Xi = %f   Tau = %f   Fx = %f   Fy = %f   Fz = %f\n",timestep,idx,angle_idx,R1,R6,cur_angle_type,cur_angle_abc,Xi,tau,force_idx.x,force_idx.y,force_idx.z);
				//printf("Xi = %f\n\n",idx.x);
